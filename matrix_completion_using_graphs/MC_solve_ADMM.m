...
if solver_params.verbose > 0
    fprintf('%3s\t%10s\t%10s\t%10s\t%10s\t%10s\t%10s\t%10s\n', 'iter', ...
      'primal grad', 'primal eps', 'dual grad', 'dual eps', 'objective', 'RMSE', 'MAE');
end
...
%            stat.res_dual(i), stat.eps_dual(i), stat.f_obj(i));
        fprintf('%3d\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.2f\t%10.2f\t%10.2f\n', i, ...
            res_pri(i), eps_pri(i), ...
            res_dual(i), eps_dual(i), stat.f_obj(i), stat.rmse_val(i), stat.mae_val(i));
    end
    
    %if i > 2 && stat.f_obj(i-1) - stat.f_obj(i) < TOL_ABS
    %    break
    %end
